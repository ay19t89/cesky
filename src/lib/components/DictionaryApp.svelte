<script lang="ts">
  ...
  async function restoreHistory(state?: { view?: string }): Promise<void> {
    ...
    view =
      state?.view === 'saved' || isSavedDictionaryPath() ? 'saved' : 'lookup';
    result = null;
  }
  ...
  onMount(() => {
    ...
    if (!getLookupWord() && isSavedDictionaryPath()) view = 'saved';
